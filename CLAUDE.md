...
- settle_market is permissionless (anyone can crank). admin_settle is admin-only with configurable delay (`admin_settle_delay_secs`, default 3600s, tunable via `update_config`). **Devnet automation tries settle_market first** (fetches VAA from Hermes, posts PriceUpdateV2 on-chain) then falls back to admin_settle. Cron fires once at 4:07 PM ET. **Known gap**: if oracle fails at 4:07 PM, admin_settle isn't eligible until 5:00 PM (1hr delay), but the job has already exited. Stale markets need manual settlement: `npx tsx scripts/automation.ts --settle`. `HERMES_URL=https://hermes-beta.pyth.network` is required for devnet (mainnet Hermes 404s on devnet feeds). Local dev uses `update_config` to set delay to 60s for rapid cycles.
