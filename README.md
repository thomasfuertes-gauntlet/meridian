...
- [ ] **`initializeConfig` should accept feed IDs as arguments** - Feed IDs differ between mainnet and devnet (Pyth uses different IDs per cluster). Currently hardcoded in `config.rs` `default_oracle_policies()`, which requires different compiled binaries or a post-init `update_oracle_policy` instruction. Production should pass feed IDs at init time so a single binary works across clusters. Scripts already handle this at runtime via `ANCHOR_PROVIDER_URL` detection in `pyth.ts`.
